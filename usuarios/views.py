...
def registro(request):
    if request.method == 'POST':
        ...
        if not senha and not confirmar_senha:
            ...
            messages.error(request, "A senha deve conter pelo menos 6 caracteres", extra_tags='senha_error')
            has_error = True

    
        if senha != confirmar_senha:
            messages.error(request, "As senhas não coincidem", extra_tags='confirmar_senha_error')
            has_error = True
